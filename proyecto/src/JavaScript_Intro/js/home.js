...
// METODO FOR EACH DE ARREGLOS
let array = ["manzana", "pera", "fresas", "mango"];

document.write("<ul>");
arr.array.forEach(function (fruta) {
    document.write(`<li>${fruta}</li>`);
});
document.write("</ul");

let frutasItems = frutas.map(function (comida) {
    return `<li>${comida}</li>`;
});

let frutasHtml = frutas.map(function (comida) {
    return `<li>${comida}</li>`;
}).join(" ");
console.log(frutasArrHtml);


// Generar html de renglones de tabla "personas" con los metodos map y join

let renglonesMap = personas.map(function (persona) {
...
console.log(renglonesMap);
let tblBodyMap = document.getElementById("tbl_datos");
tblBodyMap.innerHTML = renglonesMap;
